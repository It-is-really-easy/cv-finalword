...
class FreqFusion(nn.Module):
    def __init__(self,
        ...
        self.content_encoder = nn.Sequential(
            nn.Conv2d(self.compressed_channels, self.compressed_channels, self.encoder_kernel, padding=int((self.encoder_kernel - 1) * self.encoder_dilation / 2), groups=self.compressed_channels),
            nn.Conv2d(self.compressed_channels, lowpass_kernel ** 2 * self.up_group * self.scale_factor * self.scale_factor, (1, 1))
        )
        ...
        if self.use_high_pass:
            self.content_encoder2 = nn.Sequential(
                nn.Conv2d(self.compressed_channels, self.compressed_channels, self.encoder_kernel, padding=int((self.encoder_kernel - 1) * self.encoder_dilation / 2), groups=self.compressed_channels),
                nn.Conv2d(self.compressed_channels, highpass_kernel ** 2 * self.up_group * self.scale_factor * self.scale_factor, (1, 1))
            )
